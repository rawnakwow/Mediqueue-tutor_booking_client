...
export default function Navbar() {
  ...
  // (Connect this to your actual src/provider/AuthProvider.jsx hook later)
  ...
  // Fixed Synchronous Update Warning Safeguard Context Hook
  useEffect(() => {
  ...
  return (
    <nav className="w-full bg-white/90 dark:bg-slate-950/90 border-b border-slate-100 dark:border-slate-800 sticky top-0 z-50 backdrop-blur-md px-6 md:px-16 py-4 transition-colors duration-300">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        {/* Mobile Hamburg Drawer Toggle Trigger Button */}
        <button 
        ...
        {/* Desktop Navigation Links Checklist Constraints Matrix */}
        ...
        {/* Account Profiling Control Handles */}
        <div className="flex items-center gap-4 relative">
          
          {/* Native Dark/Light Theme Switching Indicator Button */}
          <Button isIconOnly variant="light" radius="full" onClick={handleThemeToggle} className="text-slate-600 dark:text-slate-400">
            {isDarkMode ? (
              <svg className="w-5 h-5 text-amber-400" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 2a1 1 0 011 1v1a1 1 0 11-2 0V3a1 1 0 11-2 0V3a1 1 0 011-1zm4 8a4 4 0 11-8 0 4 4 0 018 0zm-.464 4.95l.707.707a1 1 0 001.414-1.414l-.707-.707a1 1 0 00-1.414 1.414zm2.12-10.607a1 1 0 010 1.414l-.706.707a1 1 0 11-1.414-1.414l.707-.707a1 1 0 011.414 0zM17 11a1 1 0 100-2h-1a1 1 0 100 2h1zm-7 4a1 1 0 011 1v1a1 1 0 11-2 0v-1a1 1 0 011-1zM5.05 6.464A1 1 0 106.464 5.05l-.707-.707a1 1 0 00-1.414 1.414l.707.707zm1.414 8.486l-.707.707a1 1 0 01-1.414-1.414l.707-.707a1 1 0 011.414 1.414zM4 11a1 1 0 100-2H3a1 1 0 100 2h1z" clipRule="evenodd" /></svg>
            ) : (
          ...
          {user ? (
            /* Safe Responsive Dropdown Container Wrapper */
            <div className="relative">
              <button 
                onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                className="focus:outline-none block relative z-10 transition-transform duration-200 hover:scale-105"
              >
                {/* Fixed `isBordered` Crash Bug: Replaced with standard border ring design tracking classes */}
                <Avatar 
                  color="primary" 
                  radius="full" 
                  name={user.displayName} 
                  src={user.photoURL} 
                  className="w-10 h-10 object-cover border-2 border-blue-600 dark:border-blue-400 ring-2 ring-offset-2 ring-white dark:ring-slate-950" 
                />
              </button>
                ...
                <>
                  ...
                  <div className="absolute right-0 mt-3 w-56 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-xl py-2.5 z-20 animate-fadeIn text-left">
                    <div className="px-4 py-2 border-b border-slate-50 dark:border-slate-800 mb-1.5">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Active Account Session</p>
      ...
      {/* Mobile Sub-navigation Menu Drawer Drawer */}
      {isMenuOpen && (
